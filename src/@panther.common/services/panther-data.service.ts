...
import { PantherUserService, Contributor, Organism, compareOrganism, compareGroup, compareContributor } from 'panther-form-base';
...
export class PantherDataService {
  ...
  setup() {
    ...
    setup$.subscribe((responseList) => {
      if (responseList) {
        self.pantherUserService.contributors = self.loadContributors(responseList[0]);
        self.pantherUserService.groups = self.loadGroups(responseList[1]);
        return true;
      } else {
        return false;
      }
