...
import { getEdges, Edge, getNodes, subtractNodes } from './panther-form-graph';
  ...
  adjustCC() {
    ...
    if (ccNode && !ccNode.hasValue()) {
      ...
      if (ccEdges.length > 0) {
        ...
        const rootCC = pantherFormConfig.rootNode.cc;
  ...
  adjustAnnoton() {
    ...
    switch (self.annotonType) {
      case pantherFormConfig.annotonType.options.bpOnly.name:
        const rootMF = pantherFormConfig.rootNode.mf;
        const mfNode = self.getMFNode();
